...
def ratio(x):
    x = T.switch(T.lt(x, casting(-10)),
                 -(casting(1.0) / x - casting(1.0) / x ** 3 + casting(3.0) / x ** 5 - casting(15.0) / x ** 7),
                 n_cdf(x) / n_pdf(x))
    return x
...
class Sparse_GP:
    ...
    def __init__(self, n_inducing_points, n_points, input_d, input_means, input_vars, training_targets):
        ...
        self.LParamPost = theano.shared(value=initial_value.astype(theano.config.floatX), name='LParamPost',
                                        borrow=True)
        self.mParamPost = theano.shared(value=initial_value[:, 0: 1].astype(theano.config.floatX), name='mParamPost',
                                        borrow=True)
        ...
        self.z = theano.shared(value=np.zeros((n_inducing_points, input_d)).astype(theano.config.floatX), name='z',
                               borrow=True)
        self.lvar_noise = theano.shared(value=casting(0) * np.ones(1).astype(theano.config.floatX)[0],
                                        name='lvar_noise', borrow=True)
    ...
    def compute_output(self):
        ...
        self.Kzz = compute_kernel(self.lls, self.lsf, self.z, self.z) + T.eye(self.z.shape[0]) * self.jitter * T.exp(
            self.lsf)
        ...
        self.meanCavity = T.dot(self.covCavity, casting(self.n_points - self.set_for_training) / casting(
            self.n_points) * self.mParamPost)
        ...
        if self.ignore_variances:
            ...
            if exact_output_vars:
                ...
                for j in range(1, n + 1):
                    ...
                    self.kplus = T.sqrt(casting(1.0) / casting(2 * n)) * compute_kernel(self.lls, self.lsf,
                                                                                        self.input_means + inc, self.z)
                    self.kminus = T.sqrt(casting(1.0) / casting(2 * n)) * compute_kernel(self.lls, self.lsf,
                                                                                         self.input_means - inc, self.z)
    ...
    def getLogNormalizerCavity(self):
        ...
        return casting(0.5 * self.n_inducing_points * np.log(2 * np.pi)) + casting(0.5) * T.nlinalg.LogDetPSD()(
            self.covCavity) + \
    ...
    def initialize(self):
        ...
        lls = np.log(0.5 * (np.median(dist[np.triu_indices(input_means.shape[0], 1)]) + 1e-3)) * np.ones(
            input_means.shape[1])
    ...
    def getLogZ(self):
        ...
        return -0.5 * T.log(2 * np.pi * self.output_vars) - 0.5 * (
                    self.training_targets - self.output_means) ** 2 / self.output_vars
    ...
    def compute_log_ei(self, x, incumbent):
        ...
        Kzz = compute_kernel(self.lls, self.lsf, self.z, self.z) + T.eye(self.z.shape[0]) * self.jitter * T.exp(
            self.lsf)
        ...
        meanCavity = T.dot(covCavity,
                           casting(self.n_points - self.set_for_training) / casting(self.n_points) * self.mParamPost)
    ...
    def compute_log_averaged_ei(self, x, X, randomness, incumbent):
        ...
        KXX = compute_kernel(self.lls, self.lsf, X, X) - T.dot(T.dot(KXz, KzzInv), KXz.T) + T.eye(
            X.shape[0]) * self.jitter * T.exp(self.lsf)
        ...
        Kzz_expanded = compute_kernel(self.lls, self.lsf, z_expanded, z_expanded) + T.eye(
            z_expanded.shape[0]) * self.jitter * T.exp(self.lsf)
        Kzz_expandedInv = T.nlinalg.MatrixInversePSD()(Kzz_expanded)
        v_out = T.exp(self.lsf) - T.dot(Kxz_expanded * T.dot(Kxz_expanded, Kzz_expandedInv),
