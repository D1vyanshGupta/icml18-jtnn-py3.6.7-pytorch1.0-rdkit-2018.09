...
X = np.loadtxt(os.path.join(args.processed_path, 'latent_features.txt'))
y = -np.loadtxt(os.path.join(args.processed_path, 'targets.txt'))
y = y.reshape((-1, 1))
...
logP_values = np.loadtxt(os.path.join(args.processed_path, 'logP_values.txt'))
SA_scores = np.loadtxt(os.path.join(args.processed_path, 'SA_scores.txt'))
cycle_scores = np.loadtxt(os.path.join(args.processed_path, 'cycle_scores.txt'))
...
while iteration < 5:
    ...
    for i in range(60):
        ...
        if args.use_graph_conv:
            # s = model.decode_graph_conv(tree_vec, mol_vec, prob_decode=False)
            s = model.decode_graph_conv(tree_vec, mol_vec)
        else:
