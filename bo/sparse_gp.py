...
class SparseGP:
    ...
    def __init__(self, input_means, input_vars, training_targets, n_inducing_points):
        ...
        self.original_training_targets = theano.shared(value=training_targets.astype(theano.config.floatX), borrow=True,
                                                       name='y')
        ...
        self.sparse_gp = Sparse_GP(n_inducing_points, self.n_points, self.d_input, self.input_means, self.input_vars,
    ...
    def predict(self, means_test, vars_test):
        ...
        if self.predict_function is None:
            ...
            self.predict_function = theano.function([X, Z], predictions,
                                                    givens={self.input_means: X, self.input_vars: Z})
    ...
    def train_via_LBFGS(self, input_means, input_vars, training_targets, max_iterations=500):
        ...
        e = self.getEnergy()
        energy = theano.function([X, Z, y], e,
        ...
        x_opt, y_opt, opt_info = spo.fmin_l_bfgs_b(objective, initial_params, bounds=None, iprint=1,
    ...
    def train_via_ADAM(self, input_means, input_vars, training_targets, input_means_test, input_vars_test, test_targets, \
        ...
        selected_points = np.random.choice(n_data_points, n_data_points, replace=False)[
                          0: min(n_data_points, minibatch_size)]
        ...
        process_minibatch_adam = theano.function([X, Z, y], -e, updates=adam_theano(-e, all_params, learning_rate), \
                                                 givens={self.input_means: X, self.input_vars: Z,
                                                         self.original_training_targets: y})
        ...
        for j in range(max_iterations):
            ...
            for i in range(n_batches):
                ...
                print('Epoch: {}, Mini-batch: {} of {} - Energy: {} Time: {}'.format(j, i, n_batches, current_energy,
                                                                                     elapsed_time))
                sys.stdout.flush()
    ...
    def batched_greedy_ei(self, q, lower, upper, n_samples=1):
        ...
        for i in range(1, q):
            new_point = \
            global_optimization(grid, lower, upper, function_grid, function_scalar, function_scalar_gradient)[0]
