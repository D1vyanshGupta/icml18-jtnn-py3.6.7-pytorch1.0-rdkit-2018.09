...
parser.add_argument('--training_prop_pred', action='store_true')
...
for epoch in range(args.epochs):
    ...
    loader = PropMolTreeFolder(args.train, vocab, args.use_graph_conv, args.batch_size, num_workers=5)
    ...
    for idx, batch in enumerate(loader):
        ...
        try:
            ...
            if args.training_prop_pred:
                ...
                tacc_lst.append(tacc)
                aacc_lst.append(aacc)
                prop_mean_lst.append(prop_mean)
        ...
        if args.training_prop_pred:
            meters = meters + np.array([loss.item(), kl_div, wacc * 100, tacc * 100, aacc * 100, prop_loss.item()])
        else:
            meters = meters + np.array([loss.item(), kl_div, wacc * 100, tacc * 100, aacc * 100, prop_mean.item()])

        if total_step % args.print_iter == 0:
            meters /= args.print_iter
            if args.training_prop_pred:
                print("Epoch:{}, Iteration: {}, Step: {}, Beta: {:.3f}, Loss:{:.2f}, KL: {:.2f}, Word: {:.2f}, Topo: {:.2f}, Assm: {:.2f}, Prop Loss: {:.2f}".format(
                    epoch + 1, idx + 1, total_step, beta, meters[0], meters[1], meters[2], meters[3], meters[4], meters[5]))
            else:
                print("Epoch:{}, Iteration: {}, Step: {}, Beta: {:.3f}, Loss:{:.2f}, KL: {:.2f}, Word: {:.2f}, Topo: {:.2f}, Assm: {:.2f}, Prop Mean: {:.2f}".format(
                    epoch + 1, idx + 1, total_step, beta, meters[0], meters[1], meters[2], meters[3], meters[4], meters[5]))
...
if args.training_prop_pred:
    plt.plot(np.arange(total_step), prop_loss_lst, label='Prop Prediction Loss')
else:
    plt.plot(np.arange(total_step), prop_loss_lst, label='Prop Mean')
...
if args.training_prop_pred:
    with open(args.log_dir + '/{}_prop_pred_loss.pkl'.format(model_name), 'wb') as f:
        pickle.dump(aacc_lst, f, pickle.HIGHEST_PROTOCOL)
else:
    with open(args.log_dir + '/{}_prop_pred_mean.pkl'.format(model_name), 'wb') as f:
        pickle.dump(aacc_lst, f, pickle.HIGHEST_PROTOCOL)
